class ProjectSelection:
    ...
    def forward_checking(self, project, selected_projects):
        ...
        project_tasks = self.tasks[project]
        for task in range(self.n):
            if project_tasks[task] == 1:
                # Verifica si la tarea ya ha sido realizada en otro proyecto seleccionado
                for p in selected_projects:
                    if p < len(self.tasks) and self.tasks[p][task] == 1:
                        return False
                # Verifica si todas las tareas del proyecto actual han sido realizadas
